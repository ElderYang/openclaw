...
def generate_afternoon_review(data):
    ...
    print(f"  • 北向：{nm.get('net_inflow', 0):.2f}亿 | 主力：{mf.get('total_net_inflow', 0)/10000:.2f}亿")
    ...
    print(f"  北向资金：{nm.get('net_inflow', 0):.2f}亿")
    print(f"    沪股通：{nm.get('shanghai', 0):.2f}亿")
    print(f"    深股通：{nm.get('shenzhen', 0):.2f}亿")
    print()
    
    print(f"  主力资金：{mf.get('total_net_inflow', 0)/10000:.2f}亿")
    if mf.get('top5_inflow'):
        print(f"  主力流入 Top5:")
        print(f"    {'排名':<6}{'名称':<20}{'净流入':<15}")
        print(f"    {'-'*41}")
        for i, stock in enumerate(mf['top5_inflow'][:5], 1):
            print(f"    {i:<6}{stock['name']:<20}{stock['net_inflow']/10000:+.2f}万")
    print()
...
def get_market_focus(data):
    ...
    # 北向资金情况
    if nm.get('net_inflow', 0) > 0:
        focus_points.append(f"北向净流入{nm['net_inflow']:.1f}亿")
    else:
        focus_points.append(f"北向净流出{abs(nm.get('net_inflow', 0)):.1f}亿")
